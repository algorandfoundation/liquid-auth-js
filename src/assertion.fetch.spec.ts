...
describe("assertion/fetch", async () => {
  ...
  it("(OK) should have a valid example", async () => {
    ...
    const encodedOptions = await assertion.fetch.postOptions("http://localhost:0", credentialId);
    ...
    const response = await assertion.fetch.postResponse("http://localhost:0", encodedCredential);
  ...
  it("(OK) should assert a credential", async () => {
    await Promise.all(
      requestParamFixtures.map(async (credId, index) => {
        const response = await assertion.fetch.postOptions(`https://localhost:${index}`, credId);
  ...
  describe("Negative Tests", async () => {
    ...
    it("(FAIL) should not accept invalid option responses", async () => {
      await expect(() => assertion.fetch.postOptions("ORIGIN", "")).rejects.toThrow(
